...
## Filter the Required Files (Cell DA KPIs)
...
if filtered_file_list:
    ...
    print(f"No file found with the required dates in the last 7 days.")
```
...
```python
# import & concat all the Cell DA Files
...
# Re-Shpare DA KPIs (melt)
df0 = pd.melt(df, \
              id_vars=['Date','eNodeB Name','Cell Name'], \
              value_vars=['L.UL.Interference.Avg(dBm)',
                         'Radio Network Unavailability Rate_Cell',
                         'L.E-RAB.FailEst.TNL'],
              var_name='KPIs', value_name='Value').dropna()
```

## Filter the Required Files (Cell RBH KPIs)


```python
# List all files in the path
file_list1 = glob('D:/Advance_Data_Sets/KPIs_Analysis/BH_KPIs_Cell_Level/LTE_RBH/*.zip')

# Calculate the date for three days ago
required_date1 = datetime.now() - timedelta(days=7)
required_date_str1 = required_date1.strftime("%d%m%Y")

# Filter files that match the required date or the two days before
filtered_file_list1 = [file for file in file_list1 if any((required_date1+ timedelta(days=i)).strftime("%d%m%Y") in file for i in range(7))]

if filtered_file_list1:
    print("Filtered File List:", filtered_file_list1)
else:
    print(f"No file found with the required dates in the last 7 days.")
```

## Import and Concat Cell RBH KPIs


```python
# import & concat all the Cell RBH Files
df1=pd.concat((pd.read_csv(file,skiprows=range(6),\
    ...
    parse_dates=["Date"],na_values=['NIL','/0'],\
                usecols=['Date',
                        ...
                        'DL User Thrp (Mbps)',
                        'User Avg'])\
                        for file in filtered_file_list1)).reset_index(drop=True)

# Re-Shpare RBH KPIs (melt)
df2 = pd.melt(df1, \
              id_vars=['Date','eNodeB Name','Cell Name'], \
              value_vars=['DL PRB Avg Utilization',
                        'DL User Thrp (Mbps)',
                        'User Avg'],
              var_name='KPIs', value_name='Value').dropna()
```

# Concatenate Cell DA and Cell RBH KPIs


```python
# Concatenate df0 and df2
df3 = pd.concat([df0, df2], ignore_index=True)
```
...
```python
df4 = df3.pivot_table(index=['eNodeB Name','Cell Name','KPIs'],\
...
```python
df4['Average_KPIs'] = df4.iloc[:, 4:].apply(lambda row: np.average(row.dropna()), axis=1)
```
...
os.chdir('D:/Advance_Data_Sets/KPIs_Analysis/BH_KPIs_Cell_Level/LTE_RBH')
# Output File
df4.to_excel('LTE_KPIs_WCMS.xlsx',index=False, sheet_name='LTE_KPIs')
```
...
wb = load_workbook('LTE_KPIs_WCMS.xlsx')
```
...
ws511.cell(row=12, column=5).value = 'LTE Cell Level KPIs'
```
...
file_path = "D:/Advance_Data_Sets/KPIs_Analysis/BH_KPIs_Cell_Level/LTE_RBH/LTE_KPIs_WCMS.xlsx"
