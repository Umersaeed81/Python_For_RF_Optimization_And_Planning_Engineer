...
df2 = ChainMap(dict.fromkeys(['GUJRANWALA_CLUSTER_01_Rural',
                            ...
                            'REGIONAL_CENTERAL','REGIONAL_CENTER01_215'], 'Center-1'), 
...
```python
# Calculate intervals
df3 = df1.groupby(['GCell Group','Week','Region']).apply(lambda x: pd.Series({
...
```python
df4 = df3[['GCell Group','Week','Region','CSSR','DCR', 'HSR', 'DL RQI', 'UL RQI','SDCCH GoS', 'TCH GoS', 'MoB GoS',
...
```python
# re-shape()
df6=df5.pivot_table(index=['Region','Location','Week'],\
...
```python
with pd.ExcelWriter('SLA_Target_Week.xlsx',date_format = 'dd-mm-yyyy',datetime_format='dd-mm-yyyy') as writer:
        
    # Cluster BH Quater Level
    df4.to_excel(writer,sheet_name="BH_KPIs_Week",engine='openpyxl',na_rep='N/A',index=False)
    df7.to_excel(writer,sheet_name="BH_KPIs_Week_Format",engine='openpyxl',na_rep='N/A')
```
...
```python
# Load the workbook
wb = load_workbook('SLA_Target_Week.xlsx')
```
...
## Formatting 'BH_KPIs_Week_Format' Sheet
...
ws511.cell(row=12, column=5).value = 'Cluster BH KPIs Weekly'
```
...
file_path = "D:/Advance_Data_Sets/SLA/Cluster_BH_KPIs/SLA_Target_Week.xlsx"
